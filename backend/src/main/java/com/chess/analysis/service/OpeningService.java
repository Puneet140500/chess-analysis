...
public class OpeningService {
    ...
    // FEN (position part only, no move clocks) → "ECO: Opening Name"
    private final Map<String, String> fenToOpening = new HashMap<>();

    // positionKey(fenBefore) → set of UCI moves that appear in any ECO line from that position
    private final Map<String, Set<String>> ecoBookMoves = new HashMap<>();

    // Polyglot book loaded into a sorted long[] of keys for binary search
    private long[] bookKeys;
    private int[] bookMoves;   // encoded move: (toFile<<9)|(toRank<<6)|(fromFile<<3)|fromRank + promotion bits
    ...
    public record OpeningResult(String openingName, String eco) {}

    /**
     * Returns a per-move list: each entry is the most specific opening matched
     * up to and including that move's position (carries forward when no new match).
     */
    public List<OpeningResult> findOpenings(List<String> fenPositions) {
        List<OpeningResult> results = new ArrayList<>(fenPositions.size());
        String lastName = null;
        String lastEco  = null;
        for (String fen : fenPositions) {
            String key = positionKey(fen);
            String entry = fenToOpening.get(key);
            if (entry != null) {
                int sep = entry.indexOf(':');
                lastEco  = sep > 0 ? entry.substring(0, sep).trim() : null;
                lastName = sep > 0 ? entry.substring(sep + 1).trim() : entry;
            }
            results.add(new OpeningResult(lastName, lastEco));
        }
        return results;
    }

    /** Returns true if the move (UCI) from this position appears in any ECO line or the Polyglot book. */
    public boolean isBookMove(String fen, String uciMove) {
        // Primary: check ECO move table (exact move in a named line)
        Set<String> ecoMoves = ecoBookMoves.get(positionKey(fen));
        if (ecoMoves != null && ecoMoves.contains(uciMove)) return true;

        // Secondary: Polyglot masters book
        if (bookKeys != null && bookKeys.length > 0) {
            long hash = polyglotHash(fen);
            int encodedPlayed = encodeUci(uciMove);
            if (encodedPlayed >= 0) {
                int lo = 0, hi = bookKeys.length - 1;
                while (lo <= hi) {
                    int mid = (lo + hi) >>> 1;
                    if (bookKeys[mid] < hash) lo = mid + 1;
                    else if (bookKeys[mid] > hash) hi = mid - 1;
                    else {
                        int start = mid;
                        while (start > 0 && bookKeys[start - 1] == hash) start--;
                        for (int i = start; i < bookKeys.length && bookKeys[i] == hash; i++) {
                            if (bookMoves[i] == encodedPlayed) return true;
                        }
                        return false;
                    }
                }
    ...
    private void loadEcoTable() {
        try (InputStream is = getClass().getResourceAsStream("/openings/eco.tsv");
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    // Record the final position → opening name
                    MoveList ml = new MoveList();
                    ml.loadFromSan(pgn);
                    Board board = new Board();
                    // Replay move by move, recording each fenBefore → uciMove pair
                    for (Move move : ml) {
                        String fenBefore = positionKey(board.getFen());
                        String uci = move.toString();
                        ecoBookMoves.computeIfAbsent(fenBefore, k -> new HashSet<>()).add(uci);
                        board.doMove(move);
                    }
                    fenToOpening.put(positionKey(board.getFen()), eco + ": " + name);
                } catch (Exception ignored) {}
            }
            log.info("Loaded {} ECO openings, {} book positions from ECO lines",
                    fenToOpening.size(), ecoBookMoves.size());
        } catch (Exception e) {
            log.error("Failed to load ECO table", e);
        }
    }

    private String pgnToFen(String pgn) throws Exception {
        MoveList ml = new MoveList();
        ml.loadFromSan(pgn);
        Board board = new Board();
        for (Move move : ml) board.doMove(move);
        return board.getFen();
    }

    // Use only the piece placement + side-to-move portion of the FEN as key
    private String positionKey(String fen) {
        String[] parts = fen.split(" ");
        return parts[0] + " " + (parts.length > 1 ? parts[1] : "w");
    }

    // ── Polyglot book ──────────────────────────────────────────────────────────

    private void loadPolyglotBook() {
        try (InputStream is = getClass().getResourceAsStream("/openings/openings.bin")) {
            byte[] bytes = is.readAllBytes();
            int count = bytes.length / 16;
            bookKeys  = new long[count];
            bookMoves = new int[count];
            ByteBuffer buf = ByteBuffer.wrap(bytes).order(ByteOrder.BIG_ENDIAN);
            for (int i = 0; i < count; i++) {
                bookKeys[i]  = buf.getLong();
                bookMoves[i] = buf.getShort() & 0xFFFF;
                buf.getShort(); // weight
                buf.getInt();   // learn (4 bytes)
            }
            log.info("Loaded {} Polyglot book entries", count);
        } catch (Exception e) {
    ...
    /**
     * Encode a UCI move string (e.g. "e2e4", "e7e8q") into Polyglot's 16-bit format:
     * bits 0-2: to_file, 3-5: to_rank, 6-8: from_file, 9-11: from_rank, 12-14: promotion
     */
    private int encodeUci(String uci) {
        if (uci == null || uci.length() < 4) return -1;
        ...
        return (promo << 12) | (fromRank << 9) | (fromFile << 6) | (toRank << 3) | toFile;
    }

    // ── Polyglot Zobrist hashing ───────────────────────────────────────────────

    // Official Polyglot random table (781 entries):
    // [piece 0..11][square 0..63] = indices 0..767
    // castling K/Q/k/q = 768..771, en-passant file a..h = 772..779, side to move = 780
    private static final long[] RANDOM_64 = {
    ...
    /**
     * Compute the Polyglot hash for a FEN string.
     * Polyglot piece indices: bP=0,wP=1,bN=2,wN=3,bB=4,wB=5,bR=6,wR=7,bQ=8,wQ=9,bK=10,wK=11
     * Square: a1=0..h8=63 (file + rank*8)
     */
    private long polyglotHash(String fen) {
        String[] parts = fen.split(" ");
        ...
        for (char c : placement.toCharArray()) {
            if (c == '/') { rank--; file = 0; }
                ...
                int pieceIdx = pieceIndex(c);
                if (pieceIdx >= 0) {
                    int sq = file + rank * 8;
                    hash ^= RANDOM_64[pieceIdx * 64 + sq];
                }
                file++;
            }
        }

        // Castling (indices 768..771): KQ kq
        if (castling.contains("K")) hash ^= RANDOM_64[768];
        if (castling.contains("Q")) hash ^= RANDOM_64[769];
        if (castling.contains("k")) hash ^= RANDOM_64[770];
        if (castling.contains("q")) hash ^= RANDOM_64[771];

        // En passant (indices 772..779): file a..h
        if (!enPassant.equals("-")) {
            int epFile = enPassant.charAt(0) - 'a';
            if (epFile >= 0 && epFile < 8) hash ^= RANDOM_64[772 + epFile];
        }

        // Side to move (index 780): only XOR when white to move
        if ("w".equals(sideToMove)) hash ^= RANDOM_64[780];
